...
const EmailStep = ({ 
  ...
  useEffect(() => {
    ...
    if (savedSignature && !senderName && !senderTitle && !senderEmail && !senderPhone && !senderCompany && !senderWebsite) {
      try {
        ...
        const hasSignatureData = signature.name || signature.title || signature.email || signature.phone || signature.company || signature.website;
  ...
  // Update checkbox state when signature fields have data
  useEffect(() => {
    const hasSignatureData = senderName || senderTitle || senderEmail || senderPhone || senderCompany || senderWebsite;
    if (hasSignatureData && !useProfileData) {
      // Check if this data came from localStorage (matches saved data)
      const savedSignature = localStorage.getItem('recapflow-signature');
      if (savedSignature) {
        try {
          const signature = JSON.parse(savedSignature);
          const matchesLocalStorage = (
            signature.name === senderName &&
            signature.title === senderTitle &&
            signature.email === senderEmail &&
            signature.phone === senderPhone &&
            signature.company === senderCompany &&
            signature.website === senderWebsite
          );
          if (matchesLocalStorage) {
            setUseProfileData(true);
          }
        } catch (error) {
  ...
  }, [senderName, senderTitle, senderEmail, senderPhone, senderCompany, senderWebsite, useProfileData]);
